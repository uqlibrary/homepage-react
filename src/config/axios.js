...
if (!!Cookies.get(SESSION_COOKIE_NAME) && !!Cookies.get(SESSION_USER_GROUP_COOKIE_NAME)) {
    apiClient.defaults.headers.common[TOKEN_NAME] = Cookies.get(SESSION_COOKIE_NAME);
}
...
if (process.env.NODE_ENV === 'development' && !!process.env.SESSION_COOKIE_NAME) {
    apiClient.defaults.headers.common[TOKEN_NAME] = process.env.SESSION_COOKIE_NAME;
}

apiClient.isCancel = axios.isCancel; // needed for cancelling requests and the instance created does not have this method
