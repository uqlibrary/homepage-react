...
test.describe('Test and Tag Manage Asset Types', () => {
    ...
    test('Page Pagination functions correctly', async ({ page }) => {
        ...
        await expect(page.locator('.MuiTablePagination-displayedRows')).toContainText('1–25 of 60');

        // next page
        await page.locator('.MuiTablePagination-actions button[aria-label="Go to next page"]').click();
        await expect(page.locator('.MuiTablePagination-displayedRows')).toContainText('26–50 of 60');
    ...
    test('Delete and Reassign work correctly', async ({ page }) => {
        ...
        await expect(page.getByTestId('action_dialogue-asset-types-title')).toContainText(
            locale.pages.manage.assetTypes.actionDialogue.confirmationTitle,
        );
        await expect(page.getByTestId('action_dialogue-asset-types-alert')).toContainText(
        ...
        await page.getByTestId('confirm-asset-types').click();
        await expect(page.locator('.MuiAlert-message')).toContainText('Operation failed');
    });
});
