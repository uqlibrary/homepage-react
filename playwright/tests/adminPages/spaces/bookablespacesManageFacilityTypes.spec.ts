...
test.describe('Spaces Admin - manage facility types page', () => {
    ...
    test('manage facility types appears as expected onload', async ({ page }) => {
        const addGroupButton = page.getByTestId('facility-group-add');
...
test.describe('Spaces Admin - create new group dialog', () => {
    ...
    test('dialog loads as expected', async ({ page }) => {
        ...
        // open the dialog
        await expect(page.getByTestId('facility-group-add')).toBeVisible();
        await expect(page.getByTestId('facility-group-add')).toHaveText('Add new Group');
        await page.getByTestId('facility-group-add').click();
    ...
    test('can save new group', async ({ page }) => {
        // add group dialog is not visible initially
        await expect(page.getByTestId('new-group-name')).not.toBeVisible();
        await expect(page.getByTestId('new-group-first')).not.toBeVisible();

        // edit tab initially focussed
        await expect(page.getByTestId('filter-add-edit-heading')).toBeVisible();

        // swap to 'add group' tab
        await expect(page.getByTestId('facility-group-add')).toBeVisible();
        await expect(page.getByTestId('facility-group-add')).toHaveText('Add new Group');
        await page.getByTestId('facility-group-add').click();

        // add group popup dialog is now visible
        await expect(page.getByTestId('new-group-name')).toBeVisible();
        await expect(page.getByTestId('new-group-first')).toBeVisible();
        ...
        // it has swapped to the edit tab
        await expect(page.getByTestId('filter-add-edit-heading')).toBeVisible();
        // not testing content post-save as mock data does not update
    });
    ...
    test('save new group has required fields, no fields suplied', async ({ page }) => {
        // add group dialog is not visible initially
        await expect(page.getByTestId('new-group-name')).not.toBeVisible();
        await expect(page.getByTestId('new-group-first')).not.toBeVisible();
...
test.describe('Spaces Admin - edit group dialog', () => {
    ...
    test('edit group dialog loads as expected', async ({ page }) => {
        ...
        // await expect(page.getByTestId('filter-add-edit-heading')).not.toBeVisible();
    ...
    test('has correct deletion warnings for groups with no active room usage', async ({ page }) => {
        const cancelButton = page.getByTestId('dialog-cancel-button');
        ...
        const groupIds = ['5', '7', '9'];

        for (const groupId of groupIds) {
            const groupEditButton = page.getByTestId(`edit-group-${groupId}-button`);
            await groupEditButton.click();
...
test.describe('Spaces Admin - edit facility type dialog', () => {
    ...
    test('the edit facility type dialog loads as expected', async ({ page }) => {
        ...
        await expect(page.getByTestId('dialogMessage')).toContainText(
            'This facility type will be removed from 9 Spaces if you delete it. Those Spaces will not be deleted.',
        );
    });
    ...
    test('the warning message on ZERO spaces is correct', async ({ page }) => {
        const recreationalReadingCollectionGroupEditButton = page.getByTestId('edit-facility-type-43-button');

        const hasNOSpacesWarningMessage =
            'This facility type can be deleted - it is not currently showing for any Spaces.';

        await expect(recreationalReadingCollectionGroupEditButton).toBeVisible();
        await recreationalReadingCollectionGroupEditButton.click();
        await expect(page.getByText(hasNOSpacesWarningMessage)).toBeVisible();
        await expect(page.getByTestId('warning-icon')).not.toBeVisible();
        await expect(page.getByTestId('dialog-cancel-button')).toBeVisible();
        await page.getByTestId('dialog-cancel-button').click();
    });
    test('the warning message on ONE spaces is correct', async ({ page }) => {
        // test each of one, none and several counts
        const kitchenGroupEditButton = page.getByTestId('edit-facility-type-3-button');
        ...
        await expect(kitchenGroupEditButton).toBeVisible();
        await kitchenGroupEditButton.click();
        await expect(page.getByText(hasONEspaceWarningMessage)).toBeVisible();
        await expect(page.getByTestId('warning-icon')).toBeVisible();
    });
    test('the warning message on MULTIPLE spaces is correct', async ({ page }) => {
        const adjustableDeskEditButton = page.getByTestId(`edit-facility-type-39-button`);

        const hasMULTIspaceWarningMessage =
            'This facility type will be removed from 4 Spaces if you delete it. Those Spaces will not be deleted.';

        await expect(adjustableDeskEditButton).toBeVisible();
        await adjustableDeskEditButton.click();
        await expect(page.getByText(hasMULTIspaceWarningMessage)).toBeVisible();
        await expect(page.getByTestId('warning-icon')).toBeVisible();
    });
    test('warning messages swap out correctly as we close and open dialogs', async ({ page }) => {
        const recreationalReadingCollectionGroupEditButton = page.getByTestId('edit-facility-type-43-button');
        const adjustableDeskEditButton = page.getByTestId(`edit-facility-type-39-button`);
        ...
        await expect(adjustableDeskEditButton).toBeVisible();
        await adjustableDeskEditButton.click();
        // the text inserted by opening the dialog above has been removed from the page
        await expect(page.getByText(hasNOSpacesWarningMessage)).not.toBeVisible();
...
test.describe('Spaces Admin - other pages', () => {
    test('can save new group when none current', async ({ page }) => {
        ...
        // "no groups" message present
        await expect(page.getByTestId('space-facility-types-empty-message')).toBeVisible();
        // no "order groups" tab available
        await expect(page.getByTestId('facility-group-order')).not.toBeVisible();
...
test.describe('Spaces Admin - api error handling', () => {
    ...
    test.skip('drag and drop api 500', async ({ page }) => {
        await page.goto('/admin/spaces/manage/facilitytypes?user=libSpaces&responseType=reorderError');

        // we are going to drag entry #2
        const draggableFacilityGroup = page.getByTestId('spaces-dragLandingAarea').locator('li:nth-of-type(2)');
        await expect(draggableFacilityGroup).toBeVisible();
        const draggableDestination = page.getByTestId('spaces-dragLandingAarea');
        await expect(draggableDestination).toBeVisible();

        await draggableFacilityGroup.hover();
        await page.mouse.down();
