...
test.describe('Art Trail (standalone section)', () => {
    test('renders on its own, without the shared Library chrome', async ({ page }) => {
        await page.goto('/art-trail?user=public');

        await expect(page.getByTestId('art-trail-page')).toBeVisible();
        await expect(page.getByRole('heading', { name: 'Art Trail' })).toBeVisible();
        ...
        await assertAccessibility(page, '[data-testid="art-trail-page"]');
    });
});
