...
test.describe('Digital Learning Hub View page', () => {
    test.describe('details page', () => {
        test('appears as expected', async ({ page }) => {
            ...
            await expect(
                page.locator('[data-testid="dlor-detailpage-cultural-advice-custom-indicator"]'),
            ).not.toBeVisible();
            ...
            await expect(page.locator('[data-testid="dlor-detailpage-description"]')).toHaveText(
        ...
        test('shows correct button for different types of records', async ({ page }) => {
            ...
            await expect(page.locator('[data-testid="detailpage-clicklink"]')).toHaveText(
            ...
            await page.locator('[data-testid="detailpage-demographics-button"]').click();
            await page.locator('[data-testid="demographics-cancel"]').click();
            await expect(page.locator('[data-testid="demographics-cancel"]')).not.toBeVisible();
            await page.locator('[data-testid="detailpage-notify-button"]').click();
            await page.locator('[data-testid="notifications-cancel"]').click();
            await expect(page.locator('[data-testid="notifications-cancel"]')).not.toBeVisible();
        });
        test('has expected cultural advice', async ({ page }) => {
            ...
            await expect(
                page
                    .locator('[data-testid="dlor-detailpage-cultural-advice-custom-indicator"]')
        ...
        test('can handle an error', async ({ page }) => {
            ...
            await expect(
                page
                    .locator('[data-testid="dlor-detailpage-error"]')
        ...
        test('can handle an empty result', async ({ page }) => {
            ...
            await expect(
                page
                    .locator('[data-testid="dlor-detailpage-empty"]')
    ...
    test.describe('"Access it" units show properly', () => {
        test('A watchable object shows the correct units on the Get It button', async ({ page }) => {
            ...
            await page.setViewportSize({ width: 1300, height: 1000 });
            await expect(page.locator('[data-testid="detailpage-clicklink"]')).toHaveText(
                'Access the object (video 47m 44s)',
            );
        });
    ...
    test.describe('user-level privilege', () => {
        test('the non-logged in user is prompted to login', async ({ page }) => {
            ...
            await expect(
                page
                    .locator('[data-testid="dlor-homepage-loginprompt"]')
                    .getByText(/for extra features/)
                    .first(),
            ).toBeVisible();
            await expect(page.locator('[data-testid="detailpage-notify-button"]')).toHaveAttribute(
                'aria-disabled',
                'true',
            );
            await expect(page.locator('[data-testid="detailpage-demographics-button"]')).toHaveAttribute(
                'aria-disabled',
                'true',
            );
        });
        test('Loggedin user sees demographics/notify prompt', async ({ page }) => {
            ...
            await expect(page.locator('[data-testid="detailpage-getit-button"]')).not.toBeVisible();
            await expect(
                page
                    .locator('[data-testid="detailpage-clicklink"]')
        ...
        test('Non-Admin does NOT see an edit button', async ({ page }) => {
            ...
            await expect(
                page
                    .locator('[data-testid="dlor-detailpage"]')
                    .getByText(/Advanced literature searching/)
                    .first(),
            ).toBeVisible();
            await expect(page.locator('[data-testid="detailpage-admin-edit-button"]')).not.toBeVisible();
        });
    });
    test.describe('Graduate Attribute helpers on homepage', () => {
        test('Graduate attribute detailed information shows on the index page, and can be shown / hid', async ({
            ...
            await expect(
                page
                    .locator('[data-testid="graduate-attribute-10-name"]')
            ...
            await expect(page.locator('[data-testid="graduate-attribute-11-name"]')).not.toBeVisible();
            await page.locator('#courageous-thinkers-dlor-filter-checkbox').click();
            await expect(page.locator('[data-testid="graduate-attribute-12-name"]')).not.toBeVisible();
            await page.locator('#culturally-capable-dlor-filter-checkbox').click();
            await expect(page.locator('[data-testid="graduate-attribute-13-name"]')).not.toBeVisible();
        });
    });
    test.describe('User can edit their own objects', () => {
        ...
        test('User can edit the object they own', async ({ page }) => {
            ...
            // await page.FIXME_should('have.value', today);
            await page.locator('[data-testid="dlor-form-next-button"]').click();
            await page.locator('[contenteditable]').fill(testData);
            await page.locator('[data-testid="dlor-form-next-button"]').click();
            await page.locator('[data-testid="dlor-form-next-button"]').click();
    ...
    test.describe('Component shows correct visibility information', () => {
        test('Freely available object', async ({ page }) => {
            await page.goto('digital-learning-hub/view/987y-dfgrf4-76gsg-01?user=s1111111');
            await page.setViewportSize({ width: 1300, height: 1000 });
            await expect(page.locator('[data-testid="detailpage-visibility"]')).toHaveText(
                /Anyone can access this object\./,
            );
        });
