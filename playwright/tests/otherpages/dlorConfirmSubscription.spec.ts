...
test.describe('Digital Learning Hub', () => {
    test.describe('desktop homepage visits', () => {
        ...
        test('a click on an unknown confirmation link appears as expected', async ({ page }) => {
            ...
            await expect(
                page.getByTestId('dlor-confirm-line-3').getByText(/check your email and try again/),
            ).toBeVisible();
        });
        test('handles an unexpected response type', async ({ page }) => {
            ...
            await page.setViewportSize({ width: 1300, height: 1000 });
            await expect(page.getByTestId('dlor-confirm-error')).toHaveText(
        ...
        test('an error appears as expected', async ({ page }) => {
            ...
            await page.setViewportSize({ width: 1300, height: 1000 });
            await expect(page.getByTestId('dlor-confirm-error')).toHaveText(/An error has occurred during the request/);
        });
    });
});
