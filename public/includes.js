 ...
 * note you need a #/ on local to match the live dev branch
 ...
 * https://homepage-development.library.uq.edu.au/feature-leadegroot-1/#/ will load https://assets.library.uq.edu.au/reusable-webcomponents-development/master/uq-lib-reusable.min.js
...
function getScriptLocation() {
    ...
    // list of valid branches found in reusable-webcomponents AWS pipeline deployments
    // (if not in AWS you will get a 404)
    const branchList = [
        'feature-leadegroot',
        'feature-drupal',
        'master',
    ...
    const devBranch = branchList.filter(branchName => {
        ...
        return window.location.pathname.startsWith(`/${branchName}/`) || regex.test(window.location.hash);
    });
    ...
    const localFile = new RegExp('reusablebranch=').test(window.location.hash)
...
const scriptUrl = getScriptLocation();
