context('Computer availability ', () => {
    ...
    it('has appropriate aria-labels', () => {
        ...
        cy.get('button[data-testid="computers-library-button-1"]')
            ...
            .then(ariaLabel => {
                expect(ariaLabel).to.contains('Biological Sciences Library - 108 free of');
                expect(ariaLabel).to.contains('269. Click to review each level');
            });
        ...
        cy.get('[data-testid="computers-library-1-level-4-button"]')
            .should('have.attr', 'aria-label')
            .then(ariaLabel => {
                expect(ariaLabel).to.contains('Biological Sciences Library level 4. 72 free of');
                expect(ariaLabel).to.contains('110');
                expect(ariaLabel).to.contains('computers');
            });
    });
});
